...
export const useAssetMapStore = defineStore('assetMapStore', () => {
    ...
    let assetMap: Map<string, AssetMapEntry> = reactive(new Map<string, AssetMapEntry>())
    let assetList = reactive(new Map<string, AssetList>())

    let totalAssetListValue: Ref<number> = ref(0)
    let totalAssetListPercentage: Ref<number> = ref(0)
    let totalAssetListTargetPercentage: Ref<number> = ref(0)
    let totalAssetListDeviation: Ref<number> = ref(0)

    let editGroupEntries: Ref<boolean> = ref(false)
    let selectedGroup: Ref<HoldingGroup | null> = ref(null)
    ...
    function addAssetMapEntry(uuid: string, mapEntry: AssetMapEntry): void {
        if (!assetMap.has(uuid)) {
            ...
            console.log('uuid already exists in asset map. Fired patch action instead of add action')
     ...
     * @return void
     */
    function patchAssetMapEntry(uuid: string, patchedMapEntry: AssetMapEntry): void {
        if (assetMap.has(uuid)) {
            assetMap.set(uuid, patchedMapEntry)
        } else {
            console.log('uuid already exists in asset map. Fired add action instead of patch action')
    ...
    function setAssetListTotalValue() {
        ...
        assetMap.forEach((mapEntry: AssetMapEntry) => {
            let ownedQuantity: number = 0
            let currentPrice: number = 0
            switch (mapEntry.entryType) {
                case EntryTypeEnum.PUBLIC_HOLDING:
                    let publicHolding = mapEntry as PublicHolding
                    ownedQuantity = publicHolding.ownedQuantity
                    currentPrice = getNewestPriceRecord(publicHolding.publicAsset.assetPriceRecords)
                    break
                case EntryTypeEnum.PRIVATE_HOLDING:
                    let privateHolding = mapEntry as PrivateHolding
    ...
    function setAssetListTotalDeviation() {
        ...
        assetMap.forEach((mapEntry: AssetMapEntry) => {
            switch (mapEntry.entryType) {
                case EntryTypeEnum.PUBLIC_HOLDING:
                    let publicHolding = mapEntry as PublicHolding
                    let targetPercentage = publicHolding.targetPercentage
                    ...
                    let currentPercentage = (currentValue / totalAssetListValue.value) * 100
    ...
    function sortMapEntries(sortColumn: ColumnType, sortDirection: DirectionType): void {
        assetMap = new Map(
            [...assetMap].sort((
